...
public class ProductServiceImpl implements ProductService {
	...
	public ResponseEntity<String> updateProduct(Map<String, String> requestMap) {
		...
		if (optional.isEmpty()) {
			...
			return CafeUtils.getResponseEntity("Product Updated Successsfully.", HttpStatus.OK);
		} else {
		...
		try {
			if (jwtFilter.isAdmin()) {
				...
				if (!optional.isEmpty()) {
					...
					return CafeUtils.getResponseEntity("Product status updated successfull", HttpStatus.OK);
				}return CafeUtils.getResponseEntity("Product id does not exist.", HttpStatus.OK);
			} else {
				return CafeUtils.getResponseEntity(CafeConstants.UNAUTHORIZED_ACCESS, HttpStatus.UNAUTHORIZED);
			}
	...
	public ResponseEntity<List<ProductWrapper>> getByCategory(Integer id) {
		try {
			...
			(productDao.getProductByCategory(id),HttpStatus.OK);
		} catch (Exception ex) {
			ex.printStackTrace();
		}return new ResponseEntity<>(new ArrayList<>(),HttpStatus.INTERNAL_SERVER_ERROR);
	}
